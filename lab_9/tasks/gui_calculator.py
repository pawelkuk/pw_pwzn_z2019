...
class CalculatorGUI(tk.Frame):
    ...
    def init_bottom_pad(self):
        ...
        for ii, num in enumerate(range(9, 0, -1)):
            tk.Button(
                num_pad, text=num, width=5, command=partial(self.update_var, num)
            ).grid(row=ii // 3, column=(2 - ii) % 3)
        ii += 1
        tk.Button(num_pad, text="C", width=5, command=self.clear).grid(
            row=ii // 3, column=ii % 3
        )
        ii += 1
        tk.Button(
            num_pad, text="0", width=5, command=partial(self.update_var, "0")
        ).grid(row=ii // 3, column=ii % 3)
        ii += 1
        tk.Button(num_pad, text="=", width=5, command=self.calculate_result).grid(
            row=ii // 3, column=ii % 3
        )
        tk.Button(num_pad, text="MC", width=5, command=self.clean_memory).grid(
            row=5, column=0
        )
        ii += 1
        tk.Button(num_pad, text="MR", width=5, command=self.read_from_memory).grid(
            row=5, column=1
        )
        ii += 1
        tk.Button(num_pad, text="M+", width=5, command=self.memorize).grid(
            row=5, column=2
        )
        # klawiatura operacji
        ...
        tk.Button(
            operation_pad, text=".", width=5, command=partial(self.update_var, ".")
        ).grid(row=5, column=0)
    ...
    def memorize(self):
        ...
        if state:
            self.calculator._short_memory = (
                self.variables["var_1"] or self.screen["text"]
            )
            self.calculator.memorize()
        else:
            self.calculator._short_memory = (
                self.variables["var_2"] or self.variables["var_1"]
            )
            self.calculator.memorize()
